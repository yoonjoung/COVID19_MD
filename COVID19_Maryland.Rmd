...
dtabyage<-dtabyage%>%
    mutate(
        ...
        cfr=round(100*deaths/cases, 0), 
        cfrinclusive=round(100*(probabledeaths + deaths)/cases, 0), 
        temp=max(date),
        latest=date==temp
    )%>%select(-temp)
```
...
#####__Q 7. How has mortality changed over time by age? (NEW)__
...
temp<-dtabyage%>%filter(latest==TRUE & age==80)%>%select(cfrinclusive)%>%summarize_all(funs(mean))

upperrange<-round(temp$cfrinclusive+1)
...
subplot(fig50, fig60, fig70, fig80, shareY = TRUE) %>%
        layout(
            title ="Trends of case fatality rate by age group in Maryland",
...
#####__Q 6. How has it affected population across different races? And, how has it changed over time? (NEW)__
...
plot_ly(dtafig, x=~date, y=~incidenceHispanic, name = "Hispanic", 
               type = 'scatter', mode='lines',
               line = list(color = c( "#d53e4f"))) %>% 
    add_trace(y = ~incidenceAfricanAmerican, name = "African American", line = list(color = c( "#3288bd"))) %>% 
    add_trace(y = ~incidenceWhite, name = "White", line = list(color = c( "#66c2a5"))) %>%  
    add_trace(y = ~incidenceAsian, name = "Asian", line = list(color = c( "#abdda4"))) %>%     
    layout(
        title ="Trends of cumulative incidence rate by race in Maryland",
        xaxis = list(title = ""), 
...
```
As of `r latestdateMD` 10:00AM, __`r deathsMD` COVID-19-positive deaths have occurred__, according to Maryland Department of Health. __This means `r cfrMD` percent of all confirmed cases__. Below figure shows mortality by county - in terms of both absolute number (red bars) and case fatality rate (orange bars) as of `r latestdatecounty`.  
...
As of `r latestdateMD` 10:00AM, __a total of `r totaltestMD` tests have been conducted__. There are about 6 million people in Maryland, and this means __`r testrateMD` tests have been conducted in every 1000 people__. However, __a high rate of positive test indicates that testing is still limited to those with symptoms primarily, not based on effective contact tracing__. Ideally, the positive test rate should be well below 10%. __[Both the high level and its increasing trend in Maryland is worrisome, compared to the national status](https://www.vox.com/2020/5/14/21257264/coronavirus-testing-positive-rate-covid-reopen-usa)__.

Below shows the number of new tests and the positive rate by date in Maryland.  

_Note_: [A large spike on the number of new tests on April 7th appears to be artifact of delayed processes in laboratory and/or data entry, not actual changes in the number of tests](https://www.baltimoresun.com/coronavirus/bs-md-coronavirus-update-wednesday-20200408-eexgmqygnfetdcp3xymhjpxab4-story.html).   
```{r plotTrendNewTestsMD, results='asis', fig.align="left", out.width="800px"}
dtafig<-dtaMD%>%select(date, cases, negativetest, totaltests, newtests)%>%
...
plot_ly(dtafig, x=~date, 
    ...
    add_lines(
        y=~positiverate, name="Positive rate", 
...
fig<-plot_ly(dtafig, x = ~county, y = ~cases, type="bar",
        ...
        layout(
            title ="Absolute number of total confirmed cases by county in Maryland",
...
fig<-plot_ly(dtafig, x = ~county, y = ~incidence, 
             type="bar" ) %>%
        layout(
            title ="COVID-19 cumulative incidence rate by county in Maryland",
            xaxis = list(title = "County",  
                         autotick = FALSE,
                         showticklabels = TRUE, 
                         tickfont = list(size=11), 
