...
As of `r latestdateMD` 10:00AM, __a total of `r totaltestMD` tests have been conducted. This means `r testrateMD` tests have been conducted in every 1000 people. Below shows the number of new tests by date in Maryland__ - well, only starting from March 31st. For comparison:   
...
```
COVID-19 has affected __adult population relatively evenly across different ages__. The incidence rate is highest, `r incidencetop` per 100000 population, among people between `r incidencetopage` years of age. But, the rates are about or above 90 in all age groups 30 and above. 
...
And, below detailed figure is for `r ncounty150` counties with 150 or more confirmed cases. __The number of daily new cases has increased steadily (and almost exponentially) in Baltimore county and Prince George's county__.  
...
#looping code not working to save a figure with a number ! 
for (i in 1:ncounty150){
    temp<-dtafig%>%filter(county==countylist[i])
}

    temp<-dtafig%>%filter(county==countylist[1])
    fig1<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[1]) 
    temp<-dtafig%>%filter(county==countylist[2])
    fig2<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[2]) 
    temp<-dtafig%>%filter(county==countylist[3])
    fig3<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[3]) 
    temp<-dtafig%>%filter(county==countylist[4])
    fig4<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[4]) 
    temp<-dtafig%>%filter(county==countylist[5])
    fig5<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[5]) 
    temp<-dtafig%>%filter(county==countylist[6])
    fig6<- plot_ly(temp, x=~date, y=~newcases, type = 'bar', name=countylist[6]) 
...
subplot(fig1 , fig2, fig3, fig4, fig5, fig6, 
    ...
    layout(xaxis=listx,  yaxis = listy, legend= listlegend)
```
(__Source__: New York Times' [Coronavirus in the U.S.: Latest Map and Case Count](https://github.com/nytimes/covid-19-data))
...
__Data sources__:  
1. All COVID-19 data for counties come from New York Times' [Coronavirus in the U.S.: Latest Map and Case Count](https://github.com/nytimes/covid-19-data). Accessed on April 6, 2020, 10:38AM EDT. 
2. All COVID-19 data for Maryland disaggregated by age and sex come from [Maryland COVID-19 Case Map Dashboard](https://coronavirus.maryland.gov/) by Maryland Department of Health. Accessed on April 6, 2020.      
