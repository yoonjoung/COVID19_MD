...
dtapop_USstates<-read.csv("C:/Users/YoonJoung Choi/Dropbox/0 Project/COVID19_US/DataCensusBureau/co-est2019-alldata.csv")
...
dtafig%>%
    group_by(group) %>%
    do(p = panel(.)) %>%
    subplot(nrows = 2, shareX = TRUE, titleX = FALSE, shareY = FALSE)%>%
        layout(
            title ="Trends of new daily cases per 100,000 by age, Maryland", 
            yaxis=list(title = "New daily cases per 100,000")
        )
```
_Solid line is 7-day rolling average of daily new cases per 100,000 population. <span style="color: #3288bd;">Blue segment of the line is June after reopening on June 12</span>, followed by: <span style="color: #f46d43;">July</span>, <span style="color: #66bd63;">August</span>, <span style="color: #f46d43;">September</span>, and <span style="color: #66bd63;">October</span>_.   
```{r plotTrendSincePhase2Age_kids, fig.align="left", out.width="800px", out.height="400px"}
dtafig<-dtabyage%>%
    filter(is.na(age)==FALSE)%>%
    mutate(group=agegroup)
...
```{r plotTrendSincePhase2County, results='asis', fig.align="left", out.width="800px", out.height="1000px"}
...
dtafig%>%
    ...
    subplot(nrows = 5, shareX = TRUE, shareY = TRUE)%>%
        layout(
            ...
            yaxis=list(title = "New daily cases per 100,000", range(0,40))
        )    
```
_Solid line is 7-day rolling average of daily new cases per 100,000 population. <span style="color: #3288bd;">Blue segment of the line is June after reopening on June 12</span>, followed by: <span style="color: #f46d43;">July</span>, <span style="color: #66bd63;">August</span>, <span style="color: #f46d43;">September</span>, and <span style="color: #66bd63;">October</span>_.   
...
#####__Q 8. Demographic characterstics of people with confirmed COVID-19 cases and those who died from COVID-19: how has it changed over time?__ 
...
```{r plotWeeklyTrendCases, results="asis", fig.align="left", out.width="800px"}
...
```{r plotWeeklyTrendDeaths, results="asis", fig.align="left", out.width="800px"}
dtafigage %>% filter(numberofdays==7) %>% group_by(group) %>% 
    arrange(week) %>%
