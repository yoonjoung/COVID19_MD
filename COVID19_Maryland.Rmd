...
dtabycounty<-dtabycounty%>%
    ...
    mutate(
        ...
        latesttotal=max(cases, na.rm = TRUE)#latest cumulative number
    )%>%ungroup()%>%
    arrange(county, date)%>%
    filter(county!="Maryland") 
...
```{r dtaMidAtlanticRegion, eval=FALSE}
...
dtacases<-read.csv(url)%>%
    rename(country = Country_Region,
           state = Province_State,
           county = Admin2)%>%
    filter(state=="Maryland" | state=="Virginia"| state=="District of Columbia" |
           state=="Delaware" | state=="Pennsylvania")%>%
    select(country, state, county, FIPS, Combined_Key, starts_with("X"))%>%
    gather(variable, value, starts_with("X"))%>%
    rename(date = variable,
           cases = value)%>%
    mutate(group=paste0(county,", ",state),
           group=ifelse(state=="Maryland", "MD", group),
           group=ifelse(state=="District of Columbia", "DC", group))%>%
    filter(group=="MD" | group=="DC" |
            group=="Loudon" & state=="Virginia" |
            group=="Fairfax, Virginia" |
            group=="Prince William, Virginia" |
            group=="York, Pennsylvania" |
            group=="Lancaster, Pennsylvania" |
            group=="Chester, Pennsylvania" )%>%
    select(group, date, cases)%>%
    group_by(group, date)%>%
    summarize_all(funs(sum))%>%ungroup()
...
dtadeaths<-read.csv(url)%>%
    ...
    filter(state=="Maryland" | state=="Virginia"| state=="District of Columbia" |
           state=="Delaware" | state=="Pennsylvania")%>%
    mutate(state=ifelse(state=="Maryland", "MD", state), 
           state=ifelse(state=="Virginia", "VA", state), 
           state=ifelse(state=="District of Columbia", "DC", state),  
           state=ifelse(state=="Delaware" , "DE", state), 
           state=ifelse(state=="Pennsylvania", "PA", state))%>%
    select(country, state, county, FIPS, Combined_Key, starts_with("X"))%>%
    gather(variable, value, starts_with("X"))%>%
    rename(date = variable,
           deaths = value)%>%
    mutate(group=paste(state, county),
           group=ifelse(state=="Maryland", "MD", group))%>%
    select(group, date, deaths)%>%
    group_by(group, date)%>%
    summarize_all(funs(sum))%>%ungroup()
...
dtacurveRegion<-left_join(dtacases, dtadeaths, by = c("group", "date"))%>%
    mutate(
        date=mdy(substring(date, 2)), 
        pop=0,  
        pop=ifelse(group=="Maryland, US",  6045.680, pop), 
...
dtafig<-dtabycounty%>%
    ...
    mutate(
        ...
        newtestssmooth =round(c(NA,NA,NA,rollmean(newtests, 7),NA,NA,NA), 1), 
        ...
        newcasessmooth =round(c(NA,NA,NA,rollmean(newcases, 7),NA,NA,NA), 1), 
        newcasessmooth=ifelse(group!=lag(group), NA, newcasessmooth),
...
#####__Q 10. Since the phase-2 reopening, the new cases have been going up. What group has been affected most?__ 
...
_Solid line is 7-day rolling average of daily new cases per 100,000 population. <span style="color: orange;">Orange segment of the line</span> is the first two weeks after reopening, and <span style="color: red;">red part</span> is beyond that period_.    

```{r plotTrendSincePhase2Race, results="asis", fig.align="left", out.width="800px", out.height="300px"}
dtafig<-dtabyrace%>%
    ...
    mutate(
        ...
        newcasessmooth3=ifelse(date<=("2020-06-26"), NA, newcasessmooth3),
        ...
        newcasessmoothpp3=ifelse(date<=("2020-06-26"), NA, newcasessmoothpp3)
    )%>%
    group_by(group)%>%
    mutate(
           total=max(cases)/pop,
...
panel <- . %>% 
    ...
    add_trace(
        y=~newcasessmoothpp2, name="(<2 weeks since Phase-2 reopening)",
        type='scatter',mode = 'lines', 
        marker=list(color = c( "orange"), size = 1),
        line=list(color = c( "orange"))
        )%>%   
    add_trace(
        y=~newcasessmoothpp3, name="(>=2 weeks since Phase-2 reopening)",
        type='scatter',mode = 'lines', 
        marker=list(color = c( "red"), size = 1),
    ...
    layout(
        ...
        yaxis=list(title = "New daily cases per 100,000")
        )

dtafig%>%
    group_by(group) %>%
    do(p = panel(.)) %>%
    subplot(nrows = 1, shareX = TRUE, titleX = FALSE, shareY = FALSE)%>%
        layout(
        yaxis=list(title = "New daily cases per 100,000")
        )
```
...
* It is increasing across all age groups, but __the increase is most prominent among those in their 10s and 20s__.  

_Solid line is 7-day rolling average of daily new cases per 100,000 population. <span style="color: orange;">Orange segment of the line</span> is the first two weeks after reopening, and <span style="color: red;">red part</span> is beyond that period_.        

```{r plotTrendSincePhase2Age, results="asis", fig.align="left", out.width="800px", out.height="750px"}
dtafig<-dtabyage%>%
...
* Among counties with 400 or more confirmed cases...   

_Solid line is 7-day rolling average of daily new cases per 100,000 population. <span style="color: orange;">Orange segment of the line</span> is the first two weeks after reopening, and <span style="color: red;">red part</span> is beyond that period_.    

```{r plotTrendSincePhase2County, results='asis', fig.align="left", out.width="800px", out.height="800px"}
...
dtafig<-dtabycounty%>%
    ...
    mutate(
        ...
        newtestspp=round(100000*newtests/pop, 1),
        newtestssmoothpp=round(100000*newtestssmooth/pop, 1),
...
write.csv(dtaMDCOVIDcounty_HPC_Lightening, "C:/Users/YoonJoung Choi/Dropbox/0 iSquared/iSquared_HPC_COVID/Covid-19 HPC Lightning Pilot/dtaMDHD/dtaMDCOVIDcounty.csv")

colnames(dtaMDCOVIDcounty_HPC_Lightening)

colnames(read.csv("C:/Users/YoonJoung Choi/Dropbox/0 iSquared/iSquared_HPC_COVID/Covid-19 HPC Lightning Pilot/dtaMDHD/dtaMDCOVIDcounty.csv"))
```


---

#####__Q 9. Maryland starts phase-2 reopening from June 12th. How important is social distancing?__ 

__VERY IMPORTANT!__ The state starts [phase-2 reopening](https://www.wbaltv.com/article/coronavirus-recovery-plan-maryland-stage-two-update-governor-larry-hogan/32824562) on June 12th probably for various reasons. The number of new cases has been declining (see Question 2) - _[though third highest in the country as of June 10th](https://rpubs.com/YJ_Choi/COVID19_US_supplement) when the phase-2 reopening was announced_. Test positivity rate is decreasing (see Question 4) - _though still well above 5% as of June 10th_. Hospitalization is decreasing and under the state capacity threshold. Economic reasons are important, too.    
...
Still not convinced? Think this way. As of June 10, __our number of new infections ([11 per 100,000 population](https://rpubs.com/YJ_Choi/COVID19_US_states_curve)) was about 9 times higher than the peak level in [South Korea](https://rpubs.com/YJ_Choi/COVID19_SouthKorea) (back in early March) and also higher than the peak level in Italy (back in late March), an earlier epicenter in Europe__ ([see here for more international comparisons](https://rpubs.com/YJ_Choi/KCOVID_Global)).   
...
#####__Q 8.1 What do we know about the changing demographics of people with confirmed COVID-19?__ 
...
temp<-dtaMD%>%
    ...
    mutate(notestdata=(date>as.Date("2020-03-11") & date< as.Date("2020-03-29")) ,
           positiveratesmooth =round(c(NA,NA,NA,rollmean(percent_positive, 7),NA,NA,NA), 1), 
...
dtafig<-dtabycounty%>%select(date, county, cases, latesttotal)%>%
    arrange(county, date)%>%
    mutate(
        newcases=cases-lag(cases), 
        newcases=ifelse(newcases<0, 0, newcases),         
...
2. All data on county population come from US Census Bureau's [County Population Totals: 2010-2019](https://www.census.gov/data/tables/time-series/demo/popest/2010s-counties-total.html#par_textimage). Accessed on March 29, 2020.   
3. Data on Maryland population by age and sex come from Maryland Department of Planning's [Population Estimates by Race and Hispanic Origin for July 1, 2018](https://planning.maryland.gov/MSDC/Pages/pop_estimate/CensPopEst.aspx). Accessed on April 1, 2020.    
