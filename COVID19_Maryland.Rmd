...
dtabyage<-dtabyage%>%
    mutate(
        ...
        cfrprobable=round(100*(deaths+probabledeaths)/(cases+probabledeaths), 1), 
...
dtabyrace<-dtabyrace%>%
    mutate(
        ...
        mortalityprobable=round(100000*(deaths+probabledeaths)/pop, 1), 
        temp=max(date),
        latest=date==temp
    )%>%select(-temp)
```
...
#####__Q 4. How extensive has testing been? How has the test positivity rate changed? (NEW)__ 
...
#####__Q 2. How fast has it been spreading? (NEW)__ 
